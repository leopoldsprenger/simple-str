...
double timeFuncMedian(Func f, int runs = 30) {
...
int main() {
    ...
    ssTimes["splitChar"] = timeFuncMedian([&]{
        auto v = s_csv.split(',');
        volatile size_t n = v.size(); (void)n;
    });
    ...
    ssTimes["join"] = timeFuncMedian([&]{
        ...
        volatile size_t n = out.size(); (void)n;
    });

    ssTimes["replace"] = timeFuncMedian([&]{
        auto r = s.replace("A", "B");
        volatile size_t n = r.s.size(); (void)n;
    });
    ...
    ssTimes["count"] = timeFuncMedian([&]{
        volatile int c = s.count("A"); (void)c;
    });

    ssTimes["contains"] = timeFuncMedian([&]{
        volatile bool b = s.contains("XYZ"); (void)b;
    });
    ...
    stdTimes["repeat"] = timeFuncMedian([&]{
        std::string out;
        out.reserve(300000);
        for (int i = 0; i < 100000; ++i) out += "abc";
        volatile size_t n = out.size(); (void)n;
    });

    // ---------------- Output ----------------
    std::vector<std::string> ops = {
        "splitChar","splitWS","join","replace","lower","upper",
        "strip","count","contains","repeat"
    };

    std::cout << std::setw(14) << "Operation"
              << std::setw(6)  << "Mark"
              << std::setw(12) << "SStr(ms)"
    ...
    for (const auto& op : ops) {
        ...
        bool pass =
            ((tPy == 0.0) || (tSS <= tPy * (1.0 + tolerance))) &&
            (tSS <= tStd * (1.0 + tolerance));

        std::cout << std::setw(14) << op
                  << std::setw(6)  << (pass ? "✔" : "✘")
